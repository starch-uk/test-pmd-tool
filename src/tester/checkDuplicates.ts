	...
	examples.forEach(
		...
		(example: Readonly<ExampleData>, index: Readonly<number>) => {
			...
			example.violationMarkers.forEach(
				// eslint-disable-next-line @typescript-eslint/prefer-readonly-parameter-types -- Callback parameter for forEach
				(marker: Readonly<ViolationMarker>) => {
					...
					let descriptionList = markerDescriptions.get(normalized);
					if (descriptionList === undefined) {
						descriptionList = [];
						markerDescriptions.set(normalized, descriptionList);
					}
