...
function analyzeXPath(xpath) {
	...
	const specialNodeTypes = [
		'UserClass', 'UserInterface', 'UserEnum', 'ApexFile',
		'Property', 'Parameter', 'ModifierNode', 'Annotation',
		'MapEntryNode', 'ValueWhenBlock', 'ElseWhenBlock',
		'Getter', 'Setter', 'VariableDeclaratorId'
	];
	...
	for (const specialType of specialNodeTypes) {
		const specialRegex = new RegExp(`(?:\\.//|//|\\s|/|\\(|\\[|,|\\|)${specialType}(?=\\s|$|\\[|\\(|/|\\)|,|\\||]|or|and|not|return|let)`, 'g');
	...
	const comparisonRegex = /@[A-Z][a-zA-Z]*\s*(<=|>=|!=|==|<|>|eq|ne|lt|le|gt|ge|=\s*[^=\s])/g;
	let match;
	while ((match = comparisonRegex.exec(xpath)) !== null) {
		...
		if (!comparison.includes('let ') && !comparison.match(/let\s+\$/)) {
			...
			const fullComparison = xpath.substring(Math.max(0, match.index - 20), match.index + match[0].length + 20);
			const comparisonMatch = fullComparison.match(/[@$A-Za-z][\w@$]*\s*(<=|>=|!=|==|<|>|eq|ne|lt|le|gt|ge)\s*[@$A-Za-z][\w@$]*/);
			const expr = comparisonMatch ? comparisonMatch[0].trim() : comparison;
			
			if (!conditionals.some(c => c.expression === expr && c.type === 'comparison')) {
				conditionals.push({
					type: 'comparison',
					expression: expr,
					description: expr
		...
		for (let i = afterStart; i < xpath.length; i++) {
			...
			if (!inString && (char === '"' || char === "'")) {
				...
				// At the same level (no nested structures), check for operators or predicate end
					...
					if (remaining.match(/^\s+and\s+/) || remaining.match(/^\s+or\s+/)) {
						afterEnd = i;
						break;
					}
				}
	...
	const extractNotExpressions = (xpath, startPos = 0) => {
		...
		while ((match = notPattern.exec(xpath)) !== null) {
			...
			if (parenDepth === 0) {
				...
				if (!conditionals.some(c => c.expression === innerExpr && c.type === 'not_condition')) {
					conditionals.push({
						type: 'not_condition',
	...
	const orRegex = /(?:\.\/\/|\.\/|\.|^|[^|])([A-Z][a-zA-Z]*(?:Expression|Statement|Declaration|Method|Class|Node|Literal|Object|Reference|List|Map|Type))\s+or\s+([A-Z][a-zA-Z]*(?:Expression|Statement|Declaration|Method|Class|Node|Literal|Object|Reference|List|Map|Type))|(\.\/\/[A-Z][a-zA-Z]*(?:Expression|Statement|Declaration|Method|Class|Node|Literal|Object|Reference|List|Map|Type))\s+or\s+(\.\/\/[A-Z][a-zA-Z]*(?:Expression|Statement|Declaration|Method|Class|Node|Literal|Object|Reference|List|Map|Type))/g;
	let orMatch;
	while ((orMatch = orRegex.exec(xpath)) !== null) {
		...
		if (left && right) {
			...
			[left, right].forEach((part, index) => {
				if (part && !conditionals.some(c => c.expression === part && c.type === 'or_branch')) {
	...
	const unionRegex = /\(([A-Z][a-zA-Z]*(?:Expression|Statement|Declaration|Method|Class|Node|Literal|Object|Reference|List|Map|Type))\s*\|\s*([A-Z][a-zA-Z]*(?:Expression|Statement|Declaration|Method|Class|Node|Literal|Object|Reference|List|Map|Type))\)|([A-Z][a-zA-Z]*(?:Expression|Statement|Declaration|Method|Class|Node|Literal|Object|Reference|List|Map|Type))\s+\|\s+([A-Z][a-zA-Z]*(?:Expression|Statement|Declaration|Method|Class|Node|Literal|Object|Reference|List|Map|Type))(?=\s*\)|$|\s|])/g;
	let unionMatch;
	while ((unionMatch = unionRegex.exec(xpath)) !== null) {
		...
		if (left && right) {
			// Count each branch as a separate conditional
			[left, right].forEach((part, index) => {
				if (part && !conditionals.some(c => c.expression === part && c.type === 'union_branch')) {
					conditionals.push({
						type: 'union_branch',
						expression: part,
	...
	const extractIfExpressions = (xpath, startPos = 0) => {
		...
		while ((match = ifPattern.exec(xpath)) !== null) {
			...
			if (parenDepth === 0) {
				...
				if (!conditionals.some(c => c.expression === condition && c.type === 'if_condition')) {
					conditionals.push({
						type: 'if_condition',
	...
	const quantifiedRegex = /\b(some|every)\s+\$[a-zA-Z][\w]*\s+in\s+[^\s]+\s+satisfies\s+[^)]+/gi;
	let quantMatch;
	while ((quantMatch = quantifiedRegex.exec(xpath)) !== null) {
		...
		if (satisfiesMatch) {
			...
			if (!conditionals.some(c => c.expression === satisfiesCondition && c.type === `${quantifier}_condition`)) {
				conditionals.push({
	...
	const extractBooleanFunctions = (xpath, startPos = 0) => {
		...
		while ((match = funcPattern.exec(xpath)) !== null) {
			...
			if (parenDepth === 0) {
				...
				if (!conditionals.some(c => c.expression === innerExpr && c.type === `${funcName}_condition`)) {
					conditionals.push({
...
function checkXPathHardcodedValues(xpath) {
	...
	const letMatches = xpath.matchAll(/let\s+\$[a-zA-Z][a-zA-Z0-9]*\s*:=\s*([^,\n)]+)/g);
	for (const match of letMatches) {
		...
		if (varValue.startsWith('(') && varValue.endsWith(')')) {
			...
			const arrayValues = varValue.slice(1, -1).split(',').map(v => v.trim().replace(/['"]/g, ''));
	...
	const attrValueMatches = xpath.matchAll(/@[A-Z][a-zA-Z]*\s*=\s*(?:(['"])([^'"]+)\1|([^\s\[\]\(\),@\$./]+))/g);
	for (const match of attrValueMatches) {
		...
		const acceptableHardcoded = ['true', 'false', 'null', '0', '1', '-1', 'static', 'final', 'abstract', 'override', 'public', 'private', 'protected'];
		...
		if (!acceptableHardcoded.includes(lowerValue) && !declaredVariables.has(value)) {
			// Skip method names that are core API methods (join, format, etc.)
			if (attr.includes('@MethodName') && ['join', 'format', 'valueof', 'isblank', 'isempty', 'size', 'add', 'get', 'set', 'put', 'remove', 'contains', 'indexof', 'substring', 'touppercase', 'tolowercase'].includes(lowerValue)) {
				continue;
			}
			...
			if (attrName === 'Op') {
				...
				else varName = value.toLowerCase().replace(/[^a-z]/g, '') + 'Op';
	...
	const opArrayMatches = xpath.matchAll(/@Op\s*=\s*\(\s*['"]([^'"]+)['"]\s*(?:,\s*['"]([^'"]+)['"]\s*)*\)/g);
	for (const match of opArrayMatches) {
		const fullMatch = match[0];
		const ops = fullMatch.match(/'([^']+)'|"([^"]+)"/g).map(op => op.replace(/['"]/g, ''));
		if (ops.length > 1) {
			let varName = 'comparisonOps';
			if (ops.includes('+')) varName = 'arithmeticOps';
			else if (ops.includes('++') || ops.includes('--')) varName = 'incrementOps';
			else if (ops.includes('==') || ops.includes('!=')) varName = 'equalityOps';
	...
	const numberMatches = xpath.matchAll(/(?:count|>=|<=|=|>|<)\s*([0-9]+(?:\.[0-9]+)?)/g);
	for (const match of numberMatches) {
		...
		if (shouldBeVariableNumbers.includes(number)) {
			...
			const contextEnd = Math.min(xpath.length, matchIndex + fullMatch.length + 100);
...
function checkXPathCoverageInContent(content, xpathAnalysis) {
	...
	if (xpathAnalysis.conditionals && xpathAnalysis.conditionals.length > 0) {
		xpathAnalysis.conditionals.forEach(conditional => {
			const coverageResult = checkConditionalCoverage(conditional, content, lowerContent, xpathAnalysis);
			...
			const hasActualCoverageEvidence = coverageResult.evidence.some(ev => {
				...
				return (
					...
					(evLower.includes('method calls') && (evLower.includes('0-1') || evLower.includes('2+') || evLower.includes('multi-line') || evLower.includes('single-line'))) ||
					(evLower.includes('literals') && evLower.includes('found')) ||
					(evLower.includes('expressions') && evLower.includes('found'))
				);
			});
			
			// Check if both branches are covered
			const bothBranchesCovered = coverageResult.branches.true === true && coverageResult.branches.false === true;
			
			const isActuallyCovered = coverageResult.covered && 
				hasActualCoverageEvidence &&
				!coverageResult.evidence.some(ev => ev.includes('no test content found')) &&
				bothBranchesCovered;
			
			if (isActuallyCovered) {
				coverage.coveredConditionals.add(conditional.expression || conditional.description);
 ...
 */
function checkConditionalCoverage(conditional, content, lowerContent, xpathAnalysis) {
	const result = {
		covered: false,
		evidence: [],
	...
	switch (conditional.type) {
		case 'comparison':
			result.covered = checkComparisonCoverage(conditional, content, lowerContent, result);
			break;
		case 'and_operator':
			result.covered = checkAndOperatorCoverage(conditional, content, lowerContent, result);
			break;
		case 'not_condition':
			result.covered = checkNotConditionCoverage(conditional, content, lowerContent, result);
			break;
		case 'or_branch':
			result.covered = checkOrBranchCoverage(conditional, content, lowerContent, result);
			break;
		case 'union_branch':
			result.covered = checkUnionBranchCoverage(conditional, content, lowerContent, result);
			break;
		case 'if_condition':
			result.covered = checkIfConditionCoverage(conditional, content, lowerContent, result);
			break;
		case 'some_condition':
		case 'every_condition':
			result.covered = checkQuantifiedConditionCoverage(conditional, content, lowerContent, result);
			break;
		case 'exists_condition':
		case 'empty_condition':
			result.covered = checkBooleanFunctionCoverage(conditional, content, lowerContent, result);
			break;
		default:
			...
			if (!hasTestContent) {
				result.evidence.push('Unknown conditional type - no test content found');
			} else {
				result.evidence.push('Unknown conditional type - cannot automatically verify both branches');
			}
	}
...
function checkComparisonCoverage(conditional, content, lowerContent, result) {
	...
	const attrMatch = expr.match(/@([A-Z][a-zA-Z]*)\s*(<=|>=|!=|==|<|>|eq|ne|lt|le|gt|ge)\s*/);
	if (!attrMatch) {
	...
	if (attrName === 'InputParametersSize') {
		...
		const hasOneArg = /\w+\s*\(\s*[^)]+\s*\)/.test(content) && !/\w+\s*\(\s*[^,]+\s*,\s*[^)]+\)/.test(content);
		...
		if (operator === '<=') {
			...
			if (hasZeroArgs || hasOneArg) result.evidence.push('Found method calls with 0-1 arguments (true branch)');
			if (hasMultipleArgs) result.evidence.push('Found method calls with 2+ arguments (false branch)');
	...
	if (attrName === 'BeginLine') {
		...
		if (operator === '!=') {
			...
			if (hasMultiLine) result.evidence.push('Found multi-line method calls (BeginLine != EndLine) - true branch');
			if (hasSingleLine) result.evidence.push('Found single-line method calls (BeginLine == EndLine) - false branch');
			// Only covered if BOTH branches are exercised
			result.covered = result.branches.true && result.branches.false;
			return result.covered;
		}
	...
	if (!hasTestContent) {
		...
		result.evidence.push(`Comparison ${expr} - cannot automatically verify both branches`);
	}
...
function checkAndOperatorCoverage(conditional, content, lowerContent, result) {
	...
	const fullExpr = conditional.fullExpression || (conditional.description || conditional.expression).replace(/^and\s+/, '').replace(/\s*\.\.\.\s*$/, '');
	...
	if (afterCondition.includes('@') && (afterCondition.includes('!=') || afterCondition.includes('=') || afterCondition.includes('<') || afterCondition.includes('>'))) {
		// Parse and check the specific comparison
		const attrMatch = afterCondition.match(/@([A-Z][a-zA-Z]*)\s*(<=|>=|!=|==|<|>)/);
		if (attrMatch) {
							const attrName = attrMatch[1];
							const operator = attrMatch[2];
							...
							if (attrName === 'BeginLine' && operator === '!=') {
								...
								if (hasMultiLine) evidenceMessage = 'Found multi-line code patterns (true branch)';
								if (hasSingleLine) evidenceMessage = 'Found single-line code patterns (false branch)';
								...
								if (operator === '<=') {
									...
									isCovered = result.branches.true && result.branches.false;
									if (hasZeroArgs || hasOneArg) evidenceMessage = 'Found method calls with 0-1 arguments (true branch)';
									if (hasMultipleArgs) evidenceMessage = 'Found method calls with 2+ arguments (false branch)';
								} else {
						...
						if (startIdx !== -1) {
							...
							if (notInner.includes(' or ') && (notInner.includes('NewListLiteralExpression') || notInner.includes('NewMapLiteralExpression'))) {
								...
								const hasVariableDeclarations = /(String|Integer|Boolean|List|Map|Set)\s+\w+\s*=/.test(content);
								const hasOtherCode = content.length > 100 && (hasMethodCalls || hasVariableDeclarations);
								// Check if we have substantial code that doesn't contain List/Map literal patterns
								const contentWithoutLiterals = content.replace(/new\s+List\s*</g, '').replace(/List\s*</g, '').replace(/new\s+Map\s*</g, '').replace(/Map\s*<\s*String/g, '');
								const hasNonLiteralCode = hasOtherCode && contentWithoutLiterals.length > 100 && (/\w+\s*\(/.test(contentWithoutLiterals) || /(String|Integer|Boolean|List|Map|Set)\s+\w+\s*=/.test(contentWithoutLiterals));
								...
								isCovered = result.branches.true && result.branches.false;
								if (hasEitherLiteral) {
									if (hasListLiteral) evidenceMessage = 'Found List literals (exercises false branch of not())';
									if (hasMapLiteral) evidenceMessage = 'Found Map literals (exercises false branch of not())';
								}
								if (hasNonLiteralCode) evidenceMessage = 'Found code without List/Map literals (exercises true branch of not())';
								if (!hasEitherLiteral && !hasNonLiteralCode && content.trim().length > 0) {
									evidenceMessage = 'Cannot verify both branches - need code with and without List/Map literals';
								}
								...
								const contentWithoutList = content.replace(/new\s+List\s*</g, '').replace(/List\s*</g, '');
								const hasNonListCode = hasOtherCode && contentWithoutList.length > 100 && (/\w+\s*\(/.test(contentWithoutList) || /(String|Integer|Boolean|List|Map|Set)\s+\w+\s*=/.test(contentWithoutList));
								...
								isCovered = result.branches.true && result.branches.false;
								if (hasListLiteral) evidenceMessage = 'Found List literals (exercises false branch of not())';
								if (hasNonListCode) evidenceMessage = 'Found code without List literals (exercises true branch of not())';
								if (!hasListLiteral && !hasNonListCode && content.trim().length > 0) {
									evidenceMessage = 'Cannot verify both branches - need code with and without List literals';
								}
								...
								const contentWithoutMap = content.replace(/new\s+Map\s*</g, '').replace(/Map\s*<\s*String/g, '');
								const hasNonMapCode = hasOtherCode && contentWithoutMap.length > 100 && (/\w+\s*\(/.test(contentWithoutMap) || /(String|Integer|Boolean|List|Map|Set)\s+\w+\s*=/.test(contentWithoutMap));
								...
								isCovered = result.branches.true && result.branches.false;
								if (hasMapLiteral) evidenceMessage = 'Found Map literals (exercises false branch of not())';
								if (hasNonMapCode) evidenceMessage = 'Found code without Map literals (exercises true branch of not())';
								if (!hasMapLiteral && !hasNonMapCode && content.trim().length > 0) {
									evidenceMessage = 'Cannot verify both branches - need code with and without Map literals';
								}
							} else {
								...
								if (notInner.includes('/(') && notInner.includes('|')) {
									const unionMatch = notInner.match(/([A-Z][a-zA-Z]*)\[.*?\]\/\(([A-Z][a-zA-Z]*)\s*\|\s*([A-Z][a-zA-Z]*)/);
									if (unionMatch) {
										...
										const methodCallWithMethodCall = /\w+\s*\(\s*[a-zA-Z_]\w*\s*\(/.test(content);
										const methodCallWithNewObject = /\w+\s*\(\s*new\s+[A-Z]\w*/.test(content);
										const hasPattern = methodCallWithMethodCall || methodCallWithNewObject;
										
										const simpleVarPattern = /\w+\s*\(\s*[a-zA-Z_]\w*\s*\)/.test(content);
										const literalArgPattern = /\w+\s*\(\s*['"]/.test(content) || /\w+\s*\(\s*\d+/.test(content);
										const hasNonPattern = (simpleVarPattern && !methodCallWithMethodCall && !methodCallWithNewObject) || (literalArgPattern && content.length > 100);
										...
										if (hasNonPattern) {
											if (simpleVarPattern && !methodCallWithMethodCall && !methodCallWithNewObject) {
										...
										result.branches.false = false;
										isCovered = false;
										evidenceMessage = 'and connects to not() condition - cannot verify both branches (parsing failed)';
									}
								} else {
									const nodeTypeMatch = notInner.match(/([A-Z][a-zA-Z]*(?:Expression|Statement|Declaration|Node|Block))/);
									if (nodeTypeMatch) {
										const nodeType = nodeTypeMatch[1];
										const nodeTypeLower = nodeType.toLowerCase().replace(/expression|statement|declaration/g, '');
										const hasNodeType = lowerContent.includes(nodeTypeLower) || content.includes(nodeType);
										...
										isCovered = result.branches.true && result.branches.false;
										if (hasNodeType) evidenceMessage = `Found ${nodeType} in content (exercises false branch of not())`;
										if (hasNonNodeCode) evidenceMessage = `No ${nodeType} found (exercises true branch of not())`;
									} else {
										// Check for union patterns like: ReferenceExpression[1]/(MethodCallExpression | NewObjectExpression)
										if (notInner.includes('/(') && notInner.includes('|')) {
											const unionMatch = notInner.match(/([A-Z][a-zA-Z]*)\[.*?\]\/\(([A-Z][a-zA-Z]*)\s*\|\s*([A-Z][a-zA-Z]*)/);
											...
											result.branches.false = false;
											isCovered = false;
											evidenceMessage = 'and connects to not() condition - cannot verify both branches';
										}
									}
								}
							}
							...
							result.branches.false = false;
							isCovered = false;
							evidenceMessage = 'and connects to not() condition - cannot extract condition';
						}
					} else {
						// Generic - can't verify both branches automatically
						result.branches.true = false;
	...
	if (result.branches.true !== undefined && result.branches.false !== undefined) {
	...
	if (!result.branches.true) {
		...
		result.evidence.push('and operator - cannot automatically verify both branches');
	}
	return result.covered;
}
...
function checkNotConditionCoverage(conditional, content, lowerContent, result) {
	...
	if (expr.includes('NewListLiteralExpression')) {
		const hasListLiteral = /new\s+List\s*</.test(content) || /List\s*</.test(content);
		...
		result.branches.false = hasListLiteral; // not(hasList) = false when hasList is true
		result.branches.true = hasNonListCode; // not(hasList) = true when hasList is false
		if (hasListLiteral) result.evidence.push('Found List literals (exercises false branch)');
		if (hasNonListCode) result.evidence.push('Found code without List literals (exercises true branch)');
		if (!hasListLiteral && !hasNonListCode && content.trim().length > 0) {
			result.evidence.push('Cannot verify both branches - need code with and without List literals');
		}
		// Only covered if BOTH branches are exercised
		result.covered = result.branches.true && result.branches.false;
		return result.covered;
	...
	if (expr.includes('NewMapLiteralExpression')) {
		const hasMapLiteral = /new\s+Map\s*</.test(content) || /Map\s*<\s*String/.test(content);
		...
		result.branches.false = hasMapLiteral; // not(hasMap) = false when hasMap is true
		result.branches.true = hasNonMapCode; // not(hasMap) = true when hasMap is false
		if (hasMapLiteral) result.evidence.push('Found Map literals (exercises false branch)');
		if (hasNonMapCode) result.evidence.push('Found code without Map literals (exercises true branch)');
		if (!hasMapLiteral && !hasNonMapCode && content.trim().length > 0) {
			result.evidence.push('Cannot verify both branches - need code with and without Map literals');
		}
		// Only covered if BOTH branches are exercised
		result.covered = result.branches.true && result.branches.false;
		return result.covered;
	...
	if (expr.includes('/(') && expr.includes('|')) {
		const unionMatch = expr.match(/([A-Z][a-zA-Z]*)\[.*?\]\/\(([A-Z][a-zA-Z]*(?:Expression|Statement|Declaration|Node|Block))\s*\|\s*([A-Z][a-zA-Z]*(?:Expression|Statement|Declaration|Node|Block))/);
		if (unionMatch) {
			...
			const methodCallWithMethodCall = /\w+\s*\(\s*[a-zA-Z_]\w*\s*\(/.test(content); // e.g., processValue(getId())
			const methodCallWithNewObject = /\w+\s*\(\s*new\s+[A-Z]\w*/.test(content); // e.g., processValue(new MyClass())
			const hasPattern = methodCallWithMethodCall || methodCallWithNewObject;
			...
			const simpleVarPattern = /\w+\s*\(\s*[a-zA-Z_]\w*\s*\)/.test(content); // Method call with simple identifier as arg
			const hasSimpleVar = simpleVarPattern && !methodCallWithMethodCall && !methodCallWithNewObject;
			
			// Also check for method calls with string/number literals as arguments (not method calls or new objects)
			const literalArgPattern = /\w+\s*\(\s*['"]/.test(content) || /\w+\s*\(\s*\d+/.test(content); // Method call with string/number literal
			
			// True branch: method calls with simple variables or literals (not method calls or new objects)
			const hasNonPattern = (hasSimpleVar || literalArgPattern) && content.length > 100;
			...
			if (hasPattern) {
				if (methodCallWithMethodCall) {
					result.evidence.push(`Found method calls with method call as first argument (e.g., processValue(getId())) - exercises false branch of not()`);
				}
				if (methodCallWithNewObject) {
					result.evidence.push(`Found method calls with new object as first argument (e.g., processValue(new MyClass())) - exercises false branch of not()`);
				}
			}
			if (hasNonPattern) {
				if (hasSimpleVar) {
					result.evidence.push(`Found method calls with simple variable as first argument (e.g., processValue(id)) - exercises true branch of not()`);
				}
				if (literalArgPattern) {
					result.evidence.push(`Found method calls with literal as first argument (e.g., processValue('123')) - exercises true branch of not()`);
				}
			}
			if (!hasPattern && !hasNonPattern && content.trim().length > 0) {
				// We have test content but can't verify both branches
				result.evidence.push(`Cannot verify both branches for ${parentType} union pattern`);
			}
	...
	if (expr.includes(' or ')) {
		...
		orParts.forEach(part => {
			const nodeTypeMatch = part.match(/([A-Z][a-zA-Z]*(?:Expression|Statement|Declaration|Node|Block))/);
			if (nodeTypeMatch) {
				const nodeType = nodeTypeMatch[1];
		...
		const contentWithoutListMap = content.replace(/new\s+List\s*</g, '').replace(/new\s+Map\s*</g, '').replace(/Map\s*<\s*String/g, '');
		const hasNonNodeCode = hasOtherCode && contentWithoutListMap.length > 100 && (/\w+\s*\(/.test(contentWithoutListMap) || /(String|Integer|Boolean|List|Map|Set)\s+\w+\s*=/.test(contentWithoutListMap));
		...
		if (hasAnyNodeType) {
			result.evidence.push(`Found ${foundTypes.join(' or ')} in content (exercises false branch of not())`);
		}
		if (hasNonNodeCode) {
			result.evidence.push(`Found code without ${allTypes.join(' or ')} (exercises true branch of not())`);
		}
		if (!hasAnyNodeType && !hasNonNodeCode && content.trim().length > 0) {
			result.evidence.push(`Cannot verify both branches - need code with and without ${allTypes.join(' or ')}`);
		}
	...
	const nodeTypeMatch = expr.match(/([A-Z][a-zA-Z]*(?:Expression|Statement|Declaration|Node|Block))/);
	if (nodeTypeMatch) {
		const nodeType = nodeTypeMatch[1];
		...
		if (hasNodeType) {
			result.evidence.push(`Found ${nodeType} in content (exercises false branch of not())`);
		}
		if (hasNonNodeCode) {
			result.evidence.push(`No ${nodeType} found (exercises true branch of not())`);
		}
	...
	if (content.trim().length === 0) {
		result.evidence.push(`not(${expr.substring(0, 50)}) - no test content found`);
	} else {
		result.evidence.push(`not(${expr.substring(0, 50)}) - cannot verify both branches (parsing failed)`);
	}
	return result.covered;
}
...
function checkOrBranchCoverage(conditional, content, lowerContent, result) {
	const expr = conditional.expression || conditional.description;
	const nodeType = expr.replace(/^\.\/\//, '').replace(/\s+or\s+.*$/, '').trim();
	...
	if (nodeType.includes('NewListLiteralExpression')) {
		...
		if (hasListLiteral) result.evidence.push('Found List literals (true branch)');
		if (hasMapLiteral) result.evidence.push('Found Map literals (other branch, false branch)');
		result.covered = result.branches.true && result.branches.false;
		return result.covered;
	}
	...
	if (nodeType.includes('NewMapLiteralExpression')) {
		...
		if (hasMapLiteral) result.evidence.push('Found Map literals (true branch)');
		if (hasListLiteral) result.evidence.push('Found List literals (other branch, false branch)');
		result.covered = result.branches.true && result.branches.false;
		return result.covered;
	}
	...
	const nodeTypeLower = nodeType.toLowerCase().replace(/expression|statement|declaration/g, '');
	result.branches.true = lowerContent.includes(nodeTypeLower) || content.includes(nodeType);
	result.branches.false = false; // Can't automatically verify false branch
	result.covered = false;
	if (result.branches.true) result.evidence.push(`Found ${nodeType} in content`);
	else result.evidence.push(`No ${nodeType} found - cannot verify both branches`);
	return result.covered;
}
...
function checkUnionBranchCoverage(conditional, content, lowerContent, result) {
	...
	if (nodeType.includes('MethodCallExpression')) {
		...
		if (hasMethodCall) result.evidence.push('Found method calls (true branch)');
		if (hasNewObject) result.evidence.push('Found new object expressions (other branch, false branch)');
		result.covered = result.branches.true && result.branches.false;
		return result.covered;
	}
	...
	if (nodeType.includes('NewObjectExpression')) {
		...
		if (hasNewObject) result.evidence.push('Found new object expressions (true branch)');
		if (hasMethodCall) result.evidence.push('Found method calls (other branch, false branch)');
		result.covered = result.branches.true && result.branches.false;
		return result.covered;
	}
...
function checkIfConditionCoverage(conditional, content, lowerContent, result) {
	...
	if (conditionMatch) {
		...
		if (hasTestContent) {
			result.evidence.push(`if-then-else conditional - cannot automatically verify both branches`);
		} else {
			result.evidence.push(`if-then-else conditional - no test content found`);
		}
	...
	if (!hasTestContent) {
		...
		result.evidence.push('if-then-else conditional - cannot automatically verify both branches (parsing failed)');
	}
	return result.covered;
}
 ...
 */
function checkQuantifiedConditionCoverage(conditional, content, lowerContent, result) {
	...
	if (sequenceMatch && conditionMatch) {
		...
		if (hasTestContent) {
			result.evidence.push(`${quantifier} $x in ${sequence} satisfies ${condition.substring(0, 30)} - cannot automatically verify both branches`);
		} else {
			result.evidence.push(`${quantifier} $x in ${sequence} satisfies ${condition.substring(0, 30)} - no test content found`);
		}
	...
	result.covered = false;
	if (!hasTestContent) {
		result.evidence.push(`${quantifier} conditional - no test content found`);
	} else {
		result.evidence.push(`${quantifier} conditional - cannot automatically verify both branches (parsing failed)`);
	}
	return result.covered;
}
 ...
 */
function checkBooleanFunctionCoverage(conditional, content, lowerContent, result) {
	...
	const nodeTypeMatch = innerExpr.match(/([A-Z][a-zA-Z]*(?:Expression|Statement|Declaration|Node|Block))/);
	...
	if (nodeTypeMatch) {
		...
		if (funcName === 'exists') {
			...
			if (hasNodeType) {
				result.evidence.push(`exists(${nodeType}) - found ${nodeType} in content (true branch)`);
			}
			if (!hasNodeType && hasTestContent) {
				result.evidence.push(`exists(${nodeType}) - no ${nodeType} found (false branch)`);
			}
			...
			if (!hasNodeType && hasTestContent) {
				result.evidence.push(`empty(${nodeType}) - no ${nodeType} found (true branch)`);
			}
			if (hasNodeType) {
				result.evidence.push(`empty(${nodeType}) - found ${nodeType} in content (false branch)`);
			}
		}
		
		if (!hasNodeType && !hasTestContent) {
			result.evidence.push(`${funcName}(${nodeType}) - no test content found`);
		}
		...
		result.covered = false;
		if (!hasTestContent) {
			result.evidence.push(`${funcName}() conditional - no test content found`);
		} else {
			result.evidence.push(`${funcName}() conditional - cannot automatically verify both branches (parsing failed)`);
		}
		return result.covered;
}
...
class RuleTester {
	...
	extractExamples() {
		try {
			...
			console.warn(`Warning: Could not extract examples from rule file: ${error.message}`);
			return [];
		}
	}
	...
	extractRuleMetadata() {
		try {
			...
			if (!ruleElement) {
				return { ruleName: null, message: null, description: null, xpath: null };
			}

			const ruleName = ruleElement.getAttribute('name') || null;
			const message = ruleElement.getAttribute('message') || null;
			
			const descriptionNode = ruleElement.getElementsByTagName('description')[0];
			const description = descriptionNode ? descriptionNode.textContent.trim() : null;
	...
	runQualityChecks() {
		...
		if (!ruleName || !/^[A-Z][a-zA-Z0-9]*$/.test(ruleName)) {
			issues.push(`Rule name must be PascalCase (found: "${ruleName || 'missing'}")`);
		}
		...
		if (!message) {
			...
			issues.push(`Rule message must be 80 characters or fewer (found ${message.length} characters)`);
		}
		...
		if (!description) {
			...
			const versionMatch = description.match(/Version:\s*(\d+\.\d+\.\d+)/i);
			if (!versionMatch) {
				issues.push('Rule description must include a version number in semver format (e.g., "Version: 1.0.0")');
			} else {
				const version = versionMatch[1];
				if (!/^\d+\.\d+\.\d+$/.test(version)) {
					issues.push(`Version must be in semver format (found: "${version}")`);
				}
			}
		}
		...
		if (xpath) {
			const letMatches = xpath.matchAll(/let\s+\$([a-zA-Z_][a-zA-Z0-9_]*)\s*:=\s*/g);
			...
			if (declaredVars.size > 0 && description) {
				for (const varName of declaredVars) {
					...
					const varPattern = new RegExp(`\\$\\s*${varName}\\s*:\\s*[^\\s]`, 'i');
					if (!varPattern.test(description)) {
						issues.push(`XPath variable \$${varName} must have a description entry in format '\$${varName}: ...'`);
					}
				}
			}
		}
		...
		for (const example of examples) {
			...
			for (const line of lines) {
				...
				if (trimmed.startsWith('// Violation:')) {
					violationMsg = trimmed.substring('// Violation:'.length).trim();
		...
		for (const msg of violationMessages) {
			issues.push(`Duplicate violation message found: "${msg}" (appears ${violationMessageCounts.get(msg)} times)`);
		}
		...
		for (const msg of validMessages) {
			issues.push(`Duplicate valid message found: "${msg}" (appears ${validMessageCounts.get(msg)} times)`);
		}
		...
		for (let exampleIdx = 0; exampleIdx < examples.length; exampleIdx++) {
			...
			for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
				...
				if (trimmed.startsWith('// Violation:')) {
					...
					if (lineIdx + 1 < lines.length) {
						...
						if (nextLine === '' || nextLine.startsWith('//') || /^\s*$/.test(nextLine)) {
							issues.push(`Example ${exampleIdx + 1}, line ${lineIdx + 1}: // Violation: marker must be followed by code, not empty line or comment`);
						}
					} else {
						// Violation marker at end of example with no following code
						issues.push(`Example ${exampleIdx + 1}, line ${lineIdx + 1}: // Violation: marker must be followed by code`);
					}
					continue;
					...
					if (lineIdx + 1 < lines.length) {
						...
						// Check if next line is empty, only whitespace, or only a comment
						if (nextLine === '' || nextLine.startsWith('//') || /^\s*$/.test(nextLine)) {
							issues.push(`Example ${exampleIdx + 1}, line ${lineIdx + 1}: // Valid: marker must be followed by code, not empty line or comment`);
						}
					} else {
						// Valid marker at end of example with no following code
						issues.push(`Example ${exampleIdx + 1}, line ${lineIdx + 1}: // Valid: marker must be followed by code`);
					}
					continue;
				}
				
				// Check for inline markers - these override section markers for that line
				const hasInlineMarker = trimmed.includes('// ❌') || trimmed.includes('// ✅');
				if (hasInlineMarker) {
				...
				const hasSectionMarker = currentSection !== null && lineIdx > sectionStartLine;
				...
				if (!hasMarker) {
					...
					if (codeOnly && codeOnly.length > 0) {
						...
						const isStructuralOnly = /^\s*[{}]\s*$/.test(codeOnly) || 
							/^\s*}\s*;?\s*$/.test(codeOnly) ||
							/^\s*;\s*$/.test(codeOnly) ||
							/^\s*\}\s*$/.test(codeOnly);
						...
						if (!isStructuralOnly) {
							...
							if (xpath && this.results.xpathCoverage && this.results.xpathCoverage.analysis) {
								...
								if (codeContext) {
									...
									try {
										const contextCoverage = checkXPathCoverageInContent(codeContext, this.results.xpathCoverage.analysis);
										
										// If it exercises conditionals (has branch coverage) or matches node types, it exercises XPath
										const hasConditionalCoverage = contextCoverage.conditionalCoverage && contextCoverage.conditionalCoverage.length > 0 &&
											contextCoverage.conditionalCoverage.some(c => 
												(c.branches && (c.branches.true === true || c.branches.false === true)) ||
												(c.covered === true)
											);
										
										// Also check if it matches any node types from the XPath
										const hasNodeTypeCoverage = contextCoverage.coveredNodeTypes && contextCoverage.coveredNodeTypes.size > 0;
										
										// Check if any conditionals are partially or fully covered
										const hasAnyCoverage = (contextCoverage.conditionalCoverage && contextCoverage.conditionalCoverage.length > 0) ||
											(contextCoverage.coveredConditionals && contextCoverage.coveredConditionals.size > 0);
										
										const exercisesXPath = hasConditionalCoverage || hasNodeTypeCoverage || hasAnyCoverage;
		...
		if (this.results.exampleBranchCombinations && this.results.exampleBranchCombinations.length > 0) {
			...
			branchSignatureCounts.forEach((count, sig) => {
				if (count > 1 && sig && sig.trim() !== '') { // Empty signature means no coverage, ignore
			...
			if (duplicateSignatures.length > 0) {
				...
				duplicateSignatures.forEach(dup => {
					const matchingCombos = this.results.exampleBranchCombinations
						.filter(c => (c.branchSignature || '') === dup.signature);
					
					// Build detailed output with example number, line number, and line text
					const details = matchingCombos.map(c => {
						...
						if (c.type === 'violation' && c.violationIndex !== undefined) {
							...
							const testResult = this.results.testResults.find(r => r.exampleIndex === exampleIndex);
							if (testResult && testResult.details && testResult.details[c.violationIndex]) {
								const violation = testResult.details[c.violationIndex];
								...
								if (testFileLineNum >= 0 && testFileLineNum < exampleLines.length) {
									lineNumber = testFileLineNum + 1; // Convert to 1-based for display
									lineText = exampleLines[testFileLineNum].trim();
								} else {
									// Try to find the violation by looking at test file
									const testContent = testResult.file ? fs.readFileSync(testResult.file, 'utf-8') : '';
									if (testContent) {
										const testLines = testContent.split('\n');
										if (violation.line > 0 && violation.line <= testLines.length) {
											const testLineText = testLines[violation.line - 1].trim();
											// Try to find this line in the example
											const exampleLineIndex = exampleLines.findIndex(l => l.trim() === testLineText || l.trim().includes(testLineText.split('//')[0].trim()));
											if (exampleLineIndex >= 0) {
												lineNumber = exampleLineIndex + 1;
												lineText = exampleLines[exampleLineIndex].trim();
											} else {
							...
							if (!lineText) {
								...
								const { violationMarkers } = this.parseExample(exampleContent);
								if (violationMarkers && violationMarkers.length > c.violationIndex) {
									const marker = violationMarkers[c.violationIndex];
									...
									if (lineNumber <= exampleLines.length) {
										...
										const codeLineIndex = Math.min(lineNumber, exampleLines.length - 1);
										lineText = exampleLines[codeLineIndex].trim();
										if (!lineText || lineText.startsWith('//')) {
											// If marker line is the violation, try the next line
											if (codeLineIndex + 1 < exampleLines.length) {
												lineText = exampleLines[codeLineIndex + 1].trim();
											}
										}
									}
								}
							...
							if (c.markerLine && c.markerLine > 0) {
								...
								let codeLineIndex = -1;
								for (let j = c.markerLine; j < exampleLines.length; j++) {
								...
								if (codeLineIndex >= 0 && codeLineIndex < exampleLines.length) {
									const candidateLine = exampleLines[codeLineIndex].trim();
									// For inline markers (// ✅ on same line as code), get the code part
									if (c.markerType === 'inline' && candidateLine.includes('// ✅')) {
										lineText = candidateLine.split('// ✅')[0].trim();
									} else if (candidateLine && !candidateLine.startsWith('//')) {
										...
										lineText = candidateLine || c.markerDescription || 'valid code';
									}
								} else {
									// Fallback: use marker description
									lineText = c.markerDescription || 'valid code';
								}
							} else {
								// Fallback: use marker description
								lineText = c.markerDescription || 'valid code';
					...
					if (deduplicatedDetails.length > 1) {
						...
						const warningParts = [];
						warningParts.push(`The following ${deduplicatedDetails.length} marker(s) exercise the exact same branch combination:`);
						
						deduplicatedDetails.forEach(detail => {
							const markerType = detail.type === 'violation' ? 'Violation' : 
							                 detail.markerType === 'inline' ? 'Valid (inline)' :
							                 detail.markerType === 'section' ? 'Valid (section)' : 'Valid';
							
							warningParts.push(`      - Example ${detail.exampleIndex}, line ${detail.lineNumber} (${markerType}):`);
							// Truncate long lines for display
							const displayText = detail.lineText.length > 80 
								? detail.lineText.substring(0, 77) + '...'
								: detail.lineText;
							warningParts.push(`        ${displayText}`);
						});
						
						this.results.qualityChecks.warnings.push(warningParts.join('\n'));
					}
				});
			}
		}
		...
		if (this.results.exampleBranchCombinations && this.results.exampleBranchCombinations.length > 0 && process.env.DEBUG_BRANCHES) {
			console.log('\n🔍 Debug: Branch combinations tracked:');
			this.results.exampleBranchCombinations.forEach(combo => {
				const typeInfo = combo.type === 'violation' ? 'violation' : combo.type === 'valid' ? 'valid' : 'unknown';
				const violationInfo = combo.violationIndex !== undefined ? `, ${typeInfo} ${combo.violationIndex + 1}` : combo.type === 'valid' ? ', valid code' : '';
				const sectionInfo = combo.sectionIndex !== undefined ? `, section ${combo.sectionIndex + 1}` : '';
				console.log(`  Example ${combo.exampleIndex + 1}${violationInfo}${sectionInfo}: ${combo.branchSignature || '(empty)'}`);
			});
			...
			console.log(`  Total branch signatures: ${branchSignatureCounts.size}`);
			...
			if (duplicateSignatures.length > 0) {
				console.log(`  Duplicate signatures found: ${duplicateSignatures.length}`);
			}
		}
	}
	...
	createBranchSignature(coverage) {
		if (!coverage || !coverage.conditionalCoverage || !Array.isArray(coverage.conditionalCoverage)) {
			return '';
		}
		...
		const branchParts = coverage.conditionalCoverage
			.filter(c => c.branches && (c.branches.true !== undefined || c.branches.false !== undefined))
			.map(c => {
				...
				const condId = (c.conditional && (c.conditional.expression || c.conditional.description)) || 
			...
			if (trimmed.startsWith('// Violation:')) {
				...
				if (!hasInlineMarkersInExample) {
					violationMarkers.push({
						...
						description: trimmed.substring('// Violation:'.length).trim()
					});
				}
			} else if (trimmed.startsWith('// Valid:')) {
				currentMode = 'valid';
				...
				if (!hasInlineMarkersInExample) {
					validMarkers.push({
						line: index + 1, // 1-based line number in the example
						description: trimmed.substring('// Valid:'.length).trim()
					});
				}
	 ...
	 */
	createTestFileFromExample(exampleContent, exampleIndex, includeViolations = true, includeValids = true) {
		...
		const hasInlineMarkers = exampleContent.includes('// ❌') || exampleContent.includes('// ✅');

		let classContent;
		const exampleHasInlineMarkers = exampleContent.includes('// ❌') || exampleContent.includes('// ✅');
		if (exampleHasInlineMarkers) {
			...
			if (includeViolations && !includeValids) {
				...
				const cleanedLines = lines
					.filter(line => !line.trim().startsWith('// Violation:') && !line.trim().startsWith('// Valid:'))
					.map(line => {
						if (line.includes('// ❌')) {
				...
				const classMatches = content.match(/^public class\s+\w+/gm) || [];
			...
			allLines.forEach((line, index) => {
				...
				if (line.includes('public') && line.includes('(') && line.includes('{')) {
					inMethod = true;
					methodHasParams = line.includes('(') && !line.includes('()');
					methodLineCount = 0;

					// For violation methods, ensure they have at least one parameter (required by AvoidOneLinerMethods rule)
					if (includeViolations && !includeValids && !methodHasParams) {
						processedLine = line.replace('()', '(Integer param)');
					}
				} else if (line.trim() === '}') {
					inMethod = false;
				} else if (inMethod && line.trim() !== '' && !line.trim().startsWith('//')) {
					methodLineCount++;
				}
	...
	async testRuleExamples() {
		...
		if (examples.length === 0) {
			throw new Error('No examples found in the rule file. Cannot perform automated testing.');
		}
		...
		for (let i = 0; i < examples.length; i++) {
			...
			const { violations: exampleViolations, valids: exampleValids, violationMarkers, validMarkers } = this.parseExample(example);
			...
			if (exampleHasViolations && exampleHasValids) {
				...
				const violationFileInfo = this.createTestFileFromExample(example, i, true, false); // violations only
				testFile = violationFileInfo.filePath;
				isMixedExampleViolationFile = true;
			} else {
				const testFileInfo = this.createTestFileFromExample(example, i, exampleHasViolations, exampleHasValids);
				testFile = testFileInfo.filePath;
			}

			try {
				const violations = await runPMD(this.ruleFilePath, testFile);
				const ruleViolations = violations.filter(v => v.rule === this.ruleName);
				...
				if (this.results.xpathCoverage && this.results.xpathCoverage.analysis) {
					try {
						...
						ruleViolations.forEach((violation, vIndex) => {
							...
							const endLine = Math.min(testLines.length, violation.line + 5);
							const violationSnippet = testLines.slice(startLine, endLine).join('\n');
							
							const violationCoverage = checkXPathCoverageInContent(violationSnippet, this.results.xpathCoverage.analysis);
							const branchSignature = this.createBranchSignature(violationCoverage);
						...
						exampleLines.forEach((line, lineIndex) => {
							if (line.trim().startsWith('// Valid:')) {
								...
								for (let j = lineIndex + 1; j < exampleLines.length; j++) {
									const trimmed = exampleLines[j].trim();
									if (trimmed.startsWith('// Violation:') || trimmed.startsWith('// Valid:')) {
										endLine = j;
										break;
									}
								}
								...
								if (endLine > startLine) {
									...
									const validSnippet = exampleLines.slice(startLine, endLine)
										.filter(line => {
											const trimmed = line.trim();
									...
									if (validSnippet.trim()) {
										...
										const testFileEndLine = Math.min(testLines.length, endLine + 1);
										const testFileSnippet = testLines.slice(testFileStartLine, testFileEndLine).join('\n');
										
										if (testFileSnippet.trim()) {
											const validCoverage = checkXPathCoverageInContent(testFileSnippet, this.results.xpathCoverage.analysis);
											const branchSignature = this.createBranchSignature(validCoverage);
											...
											this.results.exampleBranchCombinations.push({
												...
												markerDescription: line.trim().substring('// Valid:'.length).trim(),
												branchSignature: branchSignature || '',
						...
						validMarkers.forEach((marker, mIndex) => {
							if (marker.line <= exampleLines.length) {
								...
								const endLine = Math.min(exampleLines.length, marker.line + 5);
								...
								if (validSnippet.trim()) {
									...
									if (testFileSnippet.trim()) {
										...
										this.results.exampleBranchCombinations.push({
											...
											markerDescription: marker.description || 'Inline valid marker // ✅',
											branchSignature: branchSignature || '',
											coverage: validCoverage
										});
									}
						...
						if (!exampleHasViolations && actualViolationCount === 0 && validMarkers.length === 0 && !example.includes('// Valid:')) {
							...
							if (validSnippet.trim()) {
								const testFileSnippet = testLines.slice(1).join('\n'); // Skip class declaration
								...
								if (testFileSnippet.trim()) {
									...
									this.results.exampleBranchCombinations.push({
										...
										markerType: 'full',
										markerLine: 1,
										markerDescription: 'Entire example (no markers)',
										branchSignature: branchSignature || '',
										coverage: validCoverage
									});
								}
				...
				if (expectedViolationCount > 0) {
					...
					if (actualViolationCount < expectedViolationCount) {
						passed = false;
						issues.push(`Expected at least ${expectedViolationCount} violations but found ${actualViolationCount}`);
						...
						if (violationMarkers.length > 0) {
							...
							if (marker.line <= exampleLines.length) {
								const codeLine = exampleLines[marker.line - 1].trim();
					...
					passed = false;
					issues.push(`Expected no violations but found ${actualViolationCount}`);

					// For unexpected violations, show a valid code line from the example
					const exampleLines = example.split('\n');
					for (const line of exampleLines) {
						const trimmed = line.trim();
						if (trimmed && !trimmed.startsWith('//') && !trimmed.includes('class ') && !trimmed.includes('{') && !trimmed.includes('}')) {
				...
				const hasInlineMarkers = example.includes('// ❌') || example.includes('// ✅');
						...
						ruleViolations.forEach(violation => {
							...
							if (violationMarkers.length > 0) {
								...
								violationMarkers.forEach(marker => {
									...
									const expectedViolationLine = marker.line + violationLineOffset + fileLineOffset;
									// Allow some tolerance for line number mapping (e.g., blank lines, multiple lines per violation)
									if (Math.abs(violation.line - expectedViolationLine) <= 5) {
										isExpected = true;
									}
								});
							}
							...
							if (!isExpected) {
								unexpectedViolations.push(`Unexpected violation at line ${violation.line}: ${violation.message}`);
							}
						});
					}
				...
				if (validMarkers.length > 0) {
					...
					validMarkers.forEach(marker => {
						...
						const nearbyViolations = violationLines.filter(line =>
							Math.abs(line - expectedValidLine) <= 3
						);

						if (nearbyViolations.length > 0) {
							invalidValidViolations.push(`Violation found near valid code at example line ${marker.line} (${marker.description})`);
						}
					});
				...
				if (passed) {
					console.log(`  ✅ Passed (${actualViolationCount} violations found)`);
				} else {
	...
	async runCoverageTest() {
		...
		if (xpath) {
			...
			console.log(`📊 XPath Analysis: Found ${xpathAnalysis.nodeTypes.length} AST node types, ${xpathAnalysis.conditionals.length} conditionals`);
		...
		if (this.results.xpathCoverage) {
			...
			if (allContent) {
				this.results.xpathCoverage.coverage = checkXPathCoverageInContent(allContent, this.results.xpathCoverage.analysis);
			}
		}

		const passedTests = this.results.testResults.filter(r => r.passed).length;
		const totalTests = this.results.testResults.length;
		const successRate = totalTests > 0 ? ((passedTests / totalTests) * 100).toFixed(1) : '0.0';

		// Check if the rule triggered any violations at all
		const totalViolations = this.results.testResults.reduce((sum, result) => sum + result.violations, 0);
		const totalExpectedViolations = this.results.testResults.reduce((sum, result) => sum + result.expectedViolations, 0);
		const ruleTriggersViolations = totalViolations > 0;

		// Determine different types of failures
		const testsPassed = passedTests === totalTests && ruleTriggersViolations;
		const hasHardcodedIssues = this.results.hardcodedValues?.hasIssues || false;
		const hasNodeTypeCoverageIssues = this.results.xpathCoverage?.coverage ?
			this.results.xpathCoverage.coverage.missingNodeTypes.length > 0 : false;
		const hasConditionalCoverageIssues = this.results.xpathCoverage?.coverage ?
			this.results.xpathCoverage.coverage.missingConditionals.length > 0 : false;
		const hasCoverageIssues = hasNodeTypeCoverageIssues || hasConditionalCoverageIssues;
		
		// Run quality checks
		this.runQualityChecks();
		const hasQualityIssues = !this.results.qualityChecks.passed;
		
		const hasOtherIssues = hasHardcodedIssues || hasCoverageIssues || hasQualityIssues;
		...
		console.log(`  Total violations found: ${totalViolations} (expected: ${totalExpectedViolations})`);

		// Show hardcoded values section (if any)
		if (this.results.hardcodedValues && this.results.hardcodedValues.hasIssues) {
		...
		if (this.results.overallSuccess) {
			console.log('\n  ✅ All examples passed!');
			console.log('  ✅ Rule correctly identifies violations and valid code');
			console.log(`  ✅ Rule triggers violations (${totalViolations} total)`);
		} else {
			...
			if (passedTests !== totalTests) {
				...
				this.results.testResults.forEach((result, index) => {
					if (!result.passed) {
						if (result.error) {
							console.log(`    - Example ${result.exampleIndex + 1}: Error - ${result.error}`);
						} else {
							console.log(`    - Example ${result.exampleIndex + 1}: ${result.issues.join(', ')} (${result.violations} violations found)`);
							if (result.failingLines && result.failingLines.length > 0) {
			...
			if (!ruleTriggersViolations) {
				...
				console.log('     This suggests the rule may be broken or ineffective');
			}
		}
		...
		if (this.results.xpathCoverage && this.results.xpathCoverage.coverage) {
			...
			const totalCount = this.results.xpathCoverage.analysis.nodeTypes.length;
			const coveragePercent = totalCount > 0 ? ((coveredCount / totalCount) * 100).toFixed(1) : '0.0';

			console.log(`\n📊 XPath Coverage: ${coveredCount}/${totalCount} (${coveragePercent}%)`);
			if (coverage.missingNodeTypes.length > 0) {
				console.log(`  ⚠️  Missing coverage for: ${coverage.missingNodeTypes.join(', ')}`);
			}
			
			// Report conditional coverage
			const conditionalCount = this.results.xpathCoverage.analysis.conditionals.length;
			// Only count conditionals where BOTH branches are covered
			const fullyCoveredCount = coverage.conditionalCoverage.filter(c => 
				c.covered && 
				c.branches && 
			...
			const conditionalCoveragePercent = conditionalCount > 0 ? ((fullyCoveredCount / conditionalCount) * 100).toFixed(1) : '0.0';
			...
			if (conditionalCount > 0) {
				...
				if (coverage.conditionalCoverage && coverage.conditionalCoverage.length > 0) {
					const fullyCovered = coverage.conditionalCoverage.filter(c => 
						c.covered && 
						c.branches && 
						c.branches.true === true && 
						c.branches.false === true
					);
					const partialCovered = coverage.conditionalCoverage.filter(c => 
						c.covered && 
						!(c.branches && c.branches.true === true && c.branches.false === true)
					);
					const missing = coverage.conditionalCoverage.filter(c => !c.covered);
					
					if (fullyCovered.length > 0) {
						console.log(`  ✅ Covered (${fullyCovered.length}):`);
						fullyCovered.forEach((item, index) => {
							console.log(`     ${index + 1}. ${item.conditional.description}`);
							...
							if (item.branches) {
								const branches = [];
								if (item.branches.true === true) branches.push('true branch');
								if (item.branches.false === true) branches.push('false branch');
					...
					if (missing.length > 0) {
						...
						missing.forEach((item, index) => {
							...
							if (item.branches) {
								...
								if (item.branches.true !== true) missingBranches.push('true branch');
								if (item.branches.false !== true) missingBranches.push('false branch');
					...
					const partialCoverage = coverage.conditionalCoverage.filter(c => 
						c.covered && 
						!(c.branches && c.branches.true === true && c.branches.false === true)
					);
					if (partialCoverage.length > 0) {
						console.log(`  ⚠️  Partial Coverage (${partialCoverage.length} - missing one or both branches):`);
						partialCoverage.forEach((item, index) => {
							...
							if (item.branches) {
								...
								const missingBranches = [];
								if (item.branches.true === true) coveredBranches.push('true branch');
								else if (item.branches.true === false) missingBranches.push('true branch');
								if (item.branches.false === true) coveredBranches.push('false branch');
								else if (item.branches.false === false) missingBranches.push('false branch');
								if (coveredBranches.length > 0) {
									console.log(`        → Covered: ${coveredBranches.join(', ')}`);
								}
								if (missingBranches.length > 0) {
					...
					this.results.xpathCoverage.analysis.conditionals.forEach((conditional, index) => {
						const isCovered = coverage.coveredConditionals.has(conditional.expression || conditional.description);
						const status = isCovered ? '✅' : '⚠️';
						console.log(`  ${status} ${index + 1}. ${conditional.description}`);
					});
				}
				
				const partialCoverageCheck = coverage.conditionalCoverage.filter(c => 
					c.covered && 
					!(c.branches && c.branches.true === true && c.branches.false === true)
				);
				if (missingConditionalCount > 0 || partialCoverageCheck.length > 0) {
					console.log(`  ℹ️  Full coverage requires both true and false branches to be exercised`);
				}
			}
		}
		...
		const hasQualityWarnings = this.results.qualityChecks.warnings && this.results.qualityChecks.warnings.length > 0;
		if (hasQualityIssues || hasQualityWarnings) {
			...
			if (hasQualityWarnings) {
				...
				this.results.qualityChecks.warnings.forEach((warning, index) => {
...
async function main() {
	...
	if (args.length !== 1) {
		...
		console.log('It extracts examples, creates test files, and validates the rule behavior.');
		console.log('\nRequirements:');
